...
SELECT DATEDIFF(YEAR,'2007-03-28',GETDATE()) AS YEAR, DATEDIFF(MONTH,'2007-03-28',GETDATE()) AS MONTH
...
SELECT COUNT(ACTNO) AS COUNT, YEAR(ADATE) AS YEAR
FROM DEPOSIT
GROUP BY YEAR(ADATE)

--22. Display number of accounts opened in each month.
SELECT COUNT(ACTNO) AS COUNT, MONTH(ADATE) AS MONTH
FROM DEPOSIT
GROUP BY MONTH(ADATE)

--23. Display maximum amount deposited in each year. 
SELECT MAX(AMOUNT) AS MAX_AMT, YEAR(ADATE) AS YEAR
FROM DEPOSIT
GROUP BY YEAR(ADATE)
...
SELECT MIN(AMOUNT) AS MIN_AMT, MONTH(ADATE) AS MONTH
FROM DEPOSIT
GROUP BY MONTH(ADATE)

--25. Display total amount deposited in each year.
SELECT SUM(AMOUNT) AS TOTAL_AMT, YEAR(ADATE) AS YEAR
FROM DEPOSIT
GROUP BY YEAR(ADATE)
